...
# ============================================================================
steepest_descent = function(func, x0, alpha = 1, max_iterations = 1000, tol = 1e-6) {
  x = x0
  f_val = func(x)
  iter = 0
...
#      alpha_iter = as.numeric(t(grad) %*% grad / (t(grad) %*% hess_mat %*% grad))
      ...
      while (TRUE) {
        ...
        if (f_val_new < f_val - 0.01*alpha * sum(grad^2)) {
          break
        }
        alpha = alpha / 2
      }
...
result = newtons_method(f, c(5, 2))
>>>>>>> a54f369f61c11cd1e4e0591f1d54472f8b82b321
